...
## 25. Data Encryption (Included in Base Platform)

Encryption is applied end-to-end: while data is moving between the user's device and the cloud (**data in transit**), and while it is stored in databases, file stores, backups and logs (**data at rest**). The sections below explain exactly where encryption is applied, which algorithms and key lengths are used, who holds the keys, and how the client can verify it.

> **Scope note:** The encryption controls described in this section are part of the base platform. Optional enhancements such as customer-managed keys (CMK), HSM-backed keys, or additional application-level field encryption are listed under Section 26 (Additional Security Hardening) and are billed on actuals.

### 25.1 Data at Rest

Data at rest means any data that is persisted on disk, in object storage, in database storage, or in backup volumes.

#### A. Database layer — primary path (Supabase / PostgreSQL)

| Aspect | How it is protected |
|---|---|
| **Encryption algorithm** | AES-256 |
| **Encryption mode** | Transparent Data Encryption (TDE) at the storage layer; data files, backups and snapshots are encrypted |
| **Key management** | Managed by Supabase on the paid plan; keys are stored in a key-management infrastructure separate from the database host |
| **What is encrypted** | All tables, indexes, write-ahead logs (WAL), automated backups and point-in-time recovery snapshots |
| **Who can decrypt** | Only the Supabase managed infrastructure; application users never see the storage-layer key |
| **Client-visible evidence** | Supabase project settings show "Encryption at rest: enabled"; backup exports are encrypted blobs |

#### B. Database layer — alternative path (MongoDB Atlas)

| Aspect | How it is protected |
|---|---|
| **Encryption algorithm** | AES-256 |
| **Encryption mode** | Encryption at rest for data volumes, plus encrypted snapshots |
| **Key management** | MongoDB Atlas-managed keys by default; Customer-Managed Keys (CMK via AWS KMS or Azure Key Vault) available as an optional add-on |
| **What is encrypted** | Database files, replica-set storage, automated snapshots, on-demand snapshots |
| **Who can decrypt** | MongoDB Atlas infrastructure; optional CMK gives the client control over key material |

#### C. File and document storage — AWS S3 (primary path)

| Aspect | How it is protected |
|---|---|
| **Encryption algorithm** | AES-256 |
| **Encryption mode** | Server-side encryption with Amazon S3-managed keys (SSE-S3) by default; SSE-KMS available as an option |
| **What is encrypted** | KYC documents, employee photographs, uniform/asset images, invoice PDFs, export files, signed attendance evidence |
| **Bucket-level enforcement** | S3 bucket policy rejects any unencrypted PUT request; no object can be stored without encryption |
| **Access control** | Objects are never public; access is via time-limited signed URLs generated by the application server after identity and role checks |

#### D. File and document storage — Supabase Storage (alternative if database stays on Supabase)

| Aspect | How it is protected |
|---|---|
| **Encryption algorithm** | AES-256 |
| **Encryption mode** | Server-side encryption on the underlying object store |
| **What is encrypted** | Same document categories as S3 |
| **Access control** | Row-Level Security (RLS) policies in PostgreSQL govern which user can read or write each file; signed URLs are time-bound |

#### E. Backups and disaster-recovery copies

| Aspect | How it is protected |
|---|---|
| **Primary database backups** | Encrypted by the database provider (Supabase PITR / MongoDB Atlas snapshots) |
| **AWS Backup copies** | Encrypted with AES-256; stored in an isolated AWS account with Vault Lock to prevent deletion or modification |
| **Cross-region copies** | Encrypted in transit and at the destination; keys remain in the primary region's KMS unless DR CMK is configured |
| **Application-level database dumps** | Encrypted with GPG/AES before being written to the locked S3 DR bucket |

#### F. Application logs and audit data

| Aspect | How it is protected |
|---|---|
| **CloudWatch Logs** | Encrypted with AES-256 by default; optionally encrypted with a customer-managed KMS key |
| **System activity log (application)** | Stored in the database, therefore covered by database-at-rest encryption |
| **CloudTrail audit logs** | Written to an S3 bucket with SSE-KMS and log-file validation enabled; integrity checks detect tampering |

#### G. Mobile device storage

| Aspect | How it is protected |
|---|---|
| **Local attendance / punch queue** | Stored in Capacitor/Native secure storage, not plain localStorage; iOS Keychain and Android Keystore are used |
| **Biometric data** | Face ID / fingerprint templates never leave the device; the app only receives an "authentication succeeded" signal from the OS |
| **Offline data** | SQLite or encrypted preferences; sensitive fields are not cached in plain text |

### 25.2 Data in Transit

Data in transit means any data moving across a network: from browser/mobile to the application, from application to database, between AWS services, or to/from third-party integrations.

#### A. Public-facing application traffic

| Aspect | How it is protected |
|---|---|
| **Protocol** | HTTPS only |
| **TLS version** | TLS 1.2 minimum; TLS 1.3 preferred |
| **Certificate** | AWS Certificate Manager (ACM) certificate on the Application Load Balancer and CloudFront |
| **HSTS** | HTTP Strict Transport Security header enforced so browsers always use HTTPS |
| **Redirect** | Plain HTTP requests are automatically redirected to HTTPS |

#### B. API and server-function calls

| Aspect | How it is protected |
|---|---|
| **Browser ↔ App server** | All TanStack Start server functions and API routes are served over HTTPS |
| **Authentication tokens** | JWT access tokens are sent in the `Authorization: Bearer` header over TLS; refresh tokens are stored securely |
| **Mobile ↔ App server** | Same HTTPS/TLS enforcement; certificate pinning can be added as an optional hardening measure |

#### C. Application-to-database traffic

| Aspect | How it is protected |
|---|---|
| **Supabase / PostgreSQL** | Connection is over TLS; the application client verifies the server certificate |
| **MongoDB Atlas** | TLS/SSL required by default; connection strings include `ssl=true` and certificate validation |
| **Internal AWS only** | If a self-managed RDS replica is used for DR, it runs in a private subnet and traffic between ECS tasks and RDS stays inside the AWS backbone |

#### D. Internal service communication (AWS)

| Aspect | How it is protected |
|---|---|
| **ECS tasks ↔ ALB** | Traffic inside the VPC; ALB listeners use HTTPS with ACM certificates |
| **ECS tasks ↔ RDS / ElastiCache** | Private subnets; no public IP; encryption in transit enabled where supported |
| **Cross-AZ traffic** | AWS encrypts traffic that crosses availability zones within the region |

#### E. Third-party integrations

| Integration | Transit protection |
|---|---|
| **Banking / ICICI** | Bank-mandated TLS 1.2+ with mutual TLS (mTLS) where required; API keys in headers over HTTPS |
| **Tally** | HTTPS or local network TLS depending on Tally deployment |
| **Aadhaar (Karza / AuthBridge)** | Provider-mandated TLS 1.2+ and signed requests |
| **WhatsApp / SMS (Sailesh / MSG91)** | HTTPS APIs; auth tokens in headers |
| **GST** | Government NIC/GSTN TLS endpoints |
| **Google Gemini / AI Gateway** | HTTPS; API keys sent in headers |

### 25.3 Key Management

| Aspect | Implementation |
|---|---|
| **Key storage** | AWS Secrets Manager for application secrets; AWS KMS for encryption keys; Supabase/MongoDB Atlas managed keys for database storage |
| **Key rotation** | AWS KMS keys support automatic annual rotation; database provider keys are rotated by the provider per their security policy |
| **Secret rotation** | Database credentials, API keys and integration tokens are rotated on a defined schedule or immediately after a suspected compromise |
| **Access to keys** | Keys are never checked into code; developers do not have direct access to production secrets; ECS tasks receive secrets at runtime through IAM roles |
| **Audit** | Every KMS API call and Secrets Manager retrieval is logged to CloudTrail |

### 25.4 What the Platform Controls vs What the Client Controls

| Control | Platform / Vendor responsibility | Client responsibility |
|---|---|---|
| Storage-layer encryption of database and S3 | ✅ Included | — |
| TLS certificates and HTTPS enforcement | ✅ Included | — |
| Key rotation for provider-managed keys | ✅ Included | — |
| Secure storage of user passwords / OTP | ✅ Included | — |
| Customer-managed keys (CMK) | Available as optional add-on | Decide if required |
| Endpoint/device encryption on employee phones | Recommended via MDM | Provide/police MDM policy |
| Screen lock / device PIN on field officer phones | Enforced by app | Users must not disable |
| Sharing of payslips / reports outside the system | — | Client policy and training |

### 25.5 Compliance Mapping

| Requirement | How encryption satisfies it |
|---|---|
...
|---|---|
| TLS certificate on public URL | Browser padlock / SSL Labs scan |
| S3 bucket default encryption | AWS Console → S3 → Bucket → Properties → Default encryption |
| Supabase encryption at rest | Supabase project dashboard → Database → Encryption (or project settings report) |
| MongoDB Atlas encryption | Atlas project → Security → Encryption at Rest |
| CloudTrail KMS/Secrets Manager audit | AWS Console → CloudTrail → Event history → filter by `kms` or `secretsmanager` |
| Application security report | VAPT report (Section 26.5 / VAPT activity) |

### 25.7 Optional Encryption Enhancements (On Actuals)

These are not included in the base price but can be added if the client's security policy or a regulator requires them:

| Enhancement | What it adds | Typical cost impact |
|---|---|---|
| **Customer-managed keys (CMK) with AWS KMS** | Client owns the master key; provider cannot decrypt without it | KMS key + API usage: ~INR 500 – 2,000/month |
| **Application-level field encryption** | Specific fields (e.g., Aadhaar, UAN) encrypted before reaching the database | Implementation effort + minor compute overhead |
| **HSM-backed keys (AWS CloudHSM)** | Keys stored in dedicated hardware; highest assurance | CloudHSM instance: ~INR 8,000 – 15,000/month |
| **Certificate pinning on mobile apps** | Mobile app pins the server certificate to prevent MITM attacks | One-time mobile hardening effort |
| **End-to-end encrypted document upload** | Files encrypted on the client before upload; server never sees plaintext | Implementation effort + client-side key handling |

---

## 26. Additional Security Hardening (Not Included — On Actuals)
