...
DROP TRIGGER IF EXISTS trg_notify_rehire_current_approver ON public.rehire_requests;
CREATE TRIGGER trg_notify_rehire_current_approver
AFTER INSERT OR UPDATE OF current_step_order, status ON public.rehire_requests
FOR EACH ROW EXECUTE FUNCTION public.notify_rehire_current_approver();

-- Backfill: notify current approvers of every pending request that has no open alert.
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN SELECT * FROM public.rehire_requests WHERE status = 'pending' LOOP
    UPDATE public.rehire_requests SET updated_at = now() WHERE id = r.id;
  END LOOP;
END $$;