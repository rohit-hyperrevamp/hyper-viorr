ALTER TABLE public.units ADD COLUMN IF NOT EXISTS uniform_fee_amount numeric NOT NULL DEFAULT 0;
...
BEGIN
  ...
  -- Flat unit-level uniform fee takes precedence over per-item pricing
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO public.deductions (
        ...
        'Auto-created from unit setting (uniform not included in contract) on OTP acknowledgement of issuance '
          || NEW.issuance_number || '. Admin can split into monthly instalments.',
