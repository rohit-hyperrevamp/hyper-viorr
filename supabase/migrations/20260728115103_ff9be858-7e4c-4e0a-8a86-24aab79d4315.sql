...
CREATE INDEX IF NOT EXISTS idx_deductions_source_ref ON public.deductions (source_kind, source_ref);
...
BEGIN
  ...
  -- Uniform recovery only applies when the unit does NOT include uniform in the contract.
  IF cand.unit_id IS NOT NULL THEN
    SELECT COALESCE(u.uniform_included, true) INTO uniform_included
    FROM public.units u WHERE u.id = cand.unit_id;
  END IF;

  IF COALESCE(uniform_included, true) IS TRUE THEN
    RETURN NEW;
  END IF;
